...
public class Citizen implements Rescuable, Simulatable {
	...
	public void setHp(int hp) {
		...
		if (this.hp >= 100)
			...
			if (!isDead) {
				engine.log += "Citizen died at (" +getLocation().getX()+"," +getLocation().getY()+")\n";
			}
			isDead = true;
			this.hp = 0;
			...
			else if (getDisaster() instanceof Infection){
				this.getDisaster().setActive(false);
			}
		}
	}
	...
	public void struckBy(Disaster d) throws CitizenAlreadyDeadException {
		if (this.getHp() != 0) {
			...
			throw new CitizenAlreadyDeadException(d,"this citizen is Dead now ");
	}
	...
	public String toString() {
		String s = "Citizen:\nlocation: " + "(" + location.getX() + ","
				...
				+ "\nState: " + state + "\ndisaster: ";
		if (disaster == null) {
			return s + "No disasters";
		} else {
			return s + disaster.getClass().getSimpleName();
		}
