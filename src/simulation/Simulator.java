...
public class Simulator implements WorldListener {
	...
	public void nextCycle() throws IncompatibleTargetException,
			CannotTreatException, CitizenAlreadyDeadException,
		...
		for (int i = 0; i < plannedDisasters.size(); i++) {
			...
			if (d.getStartCycle() == currentCycle) {
				...
				if (d instanceof Fire)
					...
					log += d.getClass().getSimpleName() + " happened on "
							+ d.getTarget().getClass().getSimpleName()
							+ " at (" + d.getTarget().getLocation().getX()
		...
		for (int i = 0; i < buildings.size(); i++) {
			...
			if (b.getFireDamage() >= 100) {
				...
				log += c.getClass().getSimpleName() + " happened on "
						+ c.getTarget().getClass().getSimpleName() + " at ("
		...
		for (int i = 0; i < buildings.size(); i++) {
			...
			ResidentialBuilding a=buildings.get(i);
			if (!a.isCollapsed&&a.getStructuralIntegrity()==0) {
				log += "Building destroyed at (" +a.getLocation().getX()+"," +a.getLocation().getY()+")\n";
				a.isCollapsed = true;
			}
		}
		...
		window.setCycle(currentCycle);
		window.setCasualties(calculateCasualties());

		String active = "";
		for (int i = 0; i < executedDisasters.size(); i++) {
			if (executedDisasters.get(i).isActive()) {
				if(executedDisasters.get(i).getTarget() instanceof ResidentialBuilding &&! ((ResidentialBuilding)executedDisasters.get(i).getTarget()).isCollapsed){
					active += "There is "
							+ executedDisasters.get(i).getClass().getSimpleName()
							+ " on "
							+ executedDisasters.get(i).getTarget().getClass()
									.getSimpleName()
							+ " at ("
							+ executedDisasters.get(i).getTarget().getLocation()
									.getX()
							+ ","
							+ executedDisasters.get(i).getTarget().getLocation()
									.getY() + ")\n";
				...
				else if(!(executedDisasters.get(i) instanceof Collapse)) {
					active += "There is "
							+ executedDisasters.get(i).getClass().getSimpleName()
							+ " on "
							+ executedDisasters.get(i).getTarget().getClass()
		...
		window.updateActiveDisasters(active);
		if (!log.equals("")) {
			window.updateGameLog("Cycle " + currentCycle + ":\n" + log);
		}
	}

	private void handleGas(Disaster d) throws CitizenAlreadyDeadException,
			BuildingAlreadyCollapsedException {
		...
		if (b.getFireDamage() != 0) {
			...
			log += d.getClass().getSimpleName() + " happened on "
					+ d.getTarget().getClass().getSimpleName() + " at ("
	...
	private void handleFire(Disaster d) throws CitizenAlreadyDeadException,
			BuildingAlreadyCollapsedException {
		ResidentialBuilding b = (ResidentialBuilding) d.getTarget();
		if (b.getGasLevel() == 0) {
			log += d.getClass().getSimpleName() + " happened on "
